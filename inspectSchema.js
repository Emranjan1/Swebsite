...
const db = new sqlite3.Database(dbPath, sqlite3.OPEN_READONLY, (err) => {
    ...
    db.serialize(() => {
        ...
        db.all(`PRAGMA table_info('Orders')`, (err, schema) => {
            ...
            // Specifically fetch schema for the 'OrderItems' table
            console.log(`\nSchema for OrderItems:`);
            db.all(`PRAGMA table_info('OrderItems')`, (err, schema) => {
                if (err) {
                    console.error('Error fetching schema for table OrderItems:', err.message);
                    return;
                }
                console.log(schema);
            });
